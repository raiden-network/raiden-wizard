...
from installer.constants import PATHS
...
def install_eth_client(download_cache_dir: pathlib.Path, bin_path: pathlib.Path) -> None :
    ...
    if not eth_client:
        ...
        desktop_icon = user_input('Would you like to create a desktop icon for the Ethereum client?')
        if desktop_icon:
            create_desktop_icon(eth_client_path)
    else:
        # TODO: User input require input validation.
        eth_client_path = user_input('Please specify the path to the eth client: [/usr/local/bin/geth]', default=PATHS.DEFAULT_INSTALL_DIR.join(CLIENTS.GETH))
